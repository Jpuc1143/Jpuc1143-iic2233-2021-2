        ...
        while True:
            if self.menu_state == Menu.START:
                ...
                for number, tribute in enumerate(self.available_tributes.values(), start=1):
                    self.menu_options[str(number)] = (tribute.name, Menu.SHOW_CANDIDATE_TRIBUTE)
                ...
                player_tribute = copy(self.available_tributes[self.menu_options[user_input][0]])
                ...
                self.menu_options = {}
                for number, arena in enumerate(self.available_arenas, start=1):
                    self.menu_options[str(number)] = (arena.name, Menu.NEW_ARENA)
                
                self.menu_options["w"] = ("Volver", Menu.CHOOSE_TRIBUTE)
                self.menu_options["q"] = ("Salir de DCCapitolio", Menu.EXIT)

            elif self.menu_state == Menu.NEW_ARENA:
                self.arena = copy(self.available_arenas[int(user_input)-1])
                ...
                if self.arena.player_tribute.inventory:
                    ...
                    self.menu_options = {}
                    for number,item in enumerate(self.arena.player_tribute.inventory.values(), start=1):
